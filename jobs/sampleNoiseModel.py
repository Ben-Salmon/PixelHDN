...
model = ShiftNet.load_from_checkpoint('nm_checkpoint/noiseModel.ckpt')

model.to(device)
model.eval()
samples = model.sample(img_shape=(1,1,64,64)).to('cpu').numpy()[0,0,...]

imsave('results/noiseModelSamples.tif', samples)