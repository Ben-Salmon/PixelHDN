...
from noise_models.GMM_signaldependent import GMM
...
class GatedConv(nn.Module):
    def __init__(self, num_filters, kernel_size, **kwargs):
        ...
        self.s_v_tan_conv = nn.Conv2d(num_filters, num_filters, 1)
        self.s_v_sig_conv = nn.Conv2d(num_filters, num_filters, 1)
        self.s_h_tan_conv = nn.Conv2d(num_filters, num_filters, 1)
        self.s_h_sig_conv = nn.Conv2d(num_filters, num_filters, 1)
        
    def forward(self, x_v, x_h, s):
        # Signal convolutions
        s_v_tan = self.s_v_tan_conv(s)
        s_v_sig = self.s_v_sig_conv(s)
        s_h_tan = self.s_h_tan_conv(s)
        s_h_sig = self.s_h_sig_conv(s)
...
class PixelCNN(GMM):
    ...
    def __init__(self, 
        ...
        super().__init__(n_mean, n_std, s_mean, s_std, num_gaussians)
        self.depth = depth
