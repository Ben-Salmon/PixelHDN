...
class LikelihoodModule(nn.Module):
    ...
    def forward(self, input_, x, scale):
        ...
        if x is None:
            ...
            ll = self.log_likelihood(x, distr_params, scale)
        dct = {
            'mean': mean,
            'mode': mode,
            'sample': sample,
...
class NoiseModelLikelihood(LikelihoodModule):
    ...
    def log_likelihood(self, x, params, scale):
        scale = torch.exp(scale) * params['mean']
        predicted_s_denormalized = params['mean'] * self.data_std + self.data_mean
        x_denormalized = x * self.data_std + self.data_mean
        logprob=self.noiseModel.loglikelihood(x_denormalized,predicted_s_denormalized, scale)
        return logprob
