...
HDN = LadderVAE.load_from_checkpoint('dn_checkpoint/HDN.ckpt').to(device).eval()
...
for i in range(observation.shape[0]):
    img_mmse[i], samples[i] = boilerplate.predict(observation[i],num_samples,HDN,gaussian_noise_std,device,tta)
